...
@Configuration
public class AmazonSqsConfigV2 {
	
    @Value("${queue.allocation.new}")
    private String NEW_ALLOCATION_QUEUE;
	
    private static final Logger logger = LoggerFactory.getLogger(AmazonSqsConfigV2.class);
    ...
    public AmazonSQS amazonSQS(@Value("${cloud.aws.credentials.accessKey}") String awsAccessKey,
        ...
        try {
            amazonSQS = AmazonSQSClientBuilder.standard()
                    ...
                    .build();
            logger.info("SQS Connection Successful:" + amazonSQS.getQueueUrl(NEW_ALLOCATION_QUEUE));
        } 
        catch (Exception e) {
